...
export default function LeadMapper() {
  ...
  const loadLeads = async () => {
    try {
      const result = await base44.entities.Lead.list('-updated_date', 200);
      setLeads(result);
    } catch (err) {
  ...
  const handleSearch = useCallback(async (query, locOverride) => {
    ...
    try {
      const res = await base44.functions.invoke('googlePlaces', {
        action: 'search',
        lat: loc.lat,
        lng: loc.lng,
        keyword: query,
  ...
  const handleZipSearch = useCallback(async (zip) => {
    ...
    setSearching(true);
    try {
      const res = await base44.functions.invoke('googlePlaces', { action: 'geocode', zip });
  ...
  const handleLogLead = useCallback(async (data) => {
    ...
    try {
      ...
      if (existing) {
        const updated = await base44.entities.Lead.update(existing.id, {
          pipeline_stage: data.pipeline_stage,
